...
export interface TreatmentPlanPrivateNote {
  id: string;
  tenant_id: string;
...
export function useTreatmentPlanPrivateNote(treatmentPlanId: string | undefined) {
  ...
  const {
    ...
    error,
    refetch,
  } = useSupabaseQuery<TreatmentPlanPrivateNote>({
    table: 'treatment_plan_private_notes',
    select: '*',
    filters: {
      treatment_plan_id: treatmentPlanId,
      created_by_profile_id: profileId,
    },
    enabled: !!treatmentPlanId && !!profileId,
  });
  ...
  const savePrivateNote = useCallback(async (noteContent: string) => {
    if (!treatmentPlanId || !tenantId || !profileId) {
  ...
  }, [treatmentPlanId, tenantId, profileId, privateNote, refetch]);
