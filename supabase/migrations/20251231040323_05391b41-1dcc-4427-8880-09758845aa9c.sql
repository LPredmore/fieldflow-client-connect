...
-- Recreate with new client_legal_name field
CREATE OR REPLACE FUNCTION public.get_staff_calendar_appointments(p_staff_id uuid, p_from_date timestamp with time zone DEFAULT (now() - '7 days'::interval), p_to_date timestamp with time zone DEFAULT (now() + '90 days'::interval))
 RETURNS TABLE(id uuid, tenant_id uuid, series_id uuid, client_id uuid, staff_id uuid, service_id uuid, start_at timestamp with time zone, end_at timestamp with time zone, status text, is_telehealth boolean, location_name text, time_zone text, created_at timestamp with time zone, updated_at timestamp with time zone, videoroom_url text, client_name text, client_legal_name text, service_name text, clinician_name text, display_date text, display_time text, display_end_time text, display_timezone text, start_year integer, start_month integer, start_day integer, start_hour integer, start_minute integer, end_hour integer, end_minute integer)
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT 
    ...
    a.updated_at,
    a.videoroom_url,
    -- Build client name: prefer preferred name, then full name (for calendar/dashboard)
    COALESCE(
      NULLIF(c.pat_name_preferred, ''),
      NULLIF(CONCAT_WS(' ', c.pat_name_f, c.pat_name_m, c.pat_name_l), ''),
      'Unknown Client'
    ) AS client_name,
    -- Legal name: first + last only (for clinical documentation)
    COALESCE(
      NULLIF(CONCAT_WS(' ', c.pat_name_f, c.pat_name_l), ''),
      'Unknown Client'
    ) AS client_legal_name,
    -- Service name
