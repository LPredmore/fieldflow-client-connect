-- Enhanced RPC function that returns time components for fake local Date construction
-- This allows the frontend to create Date objects where getHours() returns staff local time
...
CREATE OR REPLACE FUNCTION public.get_staff_calendar_appointments(
  p_staff_id uuid, 
  p_from_date timestamp with time zone DEFAULT (now() - '7 days'::interval), 
  p_to_date timestamp with time zone DEFAULT (now() + '90 days'::interval)
)
RETURNS TABLE(
  id uuid, 
  tenant_id uuid, 
  ...
  -- NEW: Time components for "fake local" Date construction
  start_year integer,
  start_month integer,
  start_day integer,
  start_hour integer,
...
BEGIN
  ...
  SELECT 
    ...
    -- NEW: Time components for fake local Date construction in staff's timezone
