-- Update handle_client_signup trigger to handle existing customers
-- This prevents duplicate customer records when staff creates a patient first
-- and then the patient completes their account setup
...
BEGIN
    ...
    ELSIF user_meta->>'user_type' = 'contractor' THEN
        ...
        -- Insert into clinicians table (all staff get an entry for permissions)
        -- but only clinical staff get the clinical fields populated
        INSERT INTO public.clinicians (
            ...
            CASE 
                WHEN is_clinical_staff AND user_meta->>'primary_state' IS NOT NULL 
...
-- Add helpful comment
COMMENT ON FUNCTION public.handle_client_signup() IS 
  'Handles user signup for both clients and contractors. For clients, checks if a customer record already exists (created by staff) and updates it with the new user_id instead of creating a duplicate.';